...
def main():
    ...
    optimizer_fc = torch.optimizer.Adam(model.classifier.parameters(), lr = args.lr)
    ...
    for epoch in range(args.epochs):
        train(model, input_channel, optimizer, optimizer_fc, criterion, train_loader, val_loader, epoch, writer, use_CUDA)
...
def train(model, input_channel, optimizer, optimizer_fc, criterion, train_loader, val_loader, epoch, writer, use_CUDA = True):
    model.train()
    accs = []
    losses = []
    iter_val_loader = iter(val_loader)
    ...
    for (input, label) in train_loader:
        ...
        loss_fc = (meta_criterion(output, label) * w2).sum()
        loss_fc.backward()
        optimizer_fc.step()
