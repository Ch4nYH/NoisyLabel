...
class MetaBatchNorm2d(MetaModule):
    def __init__(self, *args, **kwargs):
        ...
        if self.track_running_stats:
            self.register_buffer('running_mean',  to_var(torch.zeros(self.num_features), requires_grad=True))
            self.register_buffer('running_var',  to_var(torch.ones(self.num_features) , requires_grad=True))
        else:
            self.register_parameter('running_mean', None)
            self.register_parameter('running_var', None)
